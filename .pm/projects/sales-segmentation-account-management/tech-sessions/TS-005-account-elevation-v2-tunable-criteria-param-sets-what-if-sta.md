...
decisions:
  ...
  - "Owner assignment shipped (Austin approved the ya_customers.salesID write): Owner card on the work page assigns the REAL owner on the home row via the existing changeAccountManager(), logged as owner_assign; dropdown = 19 active salespeople + Unmanaged; same-domain splits flagged, never overwritten (decision 15 preserved). Stage machine now honest: Owner-assigned stage requires a real salesID; Confirm requires owner + Qualified. Baseline found: 355/20,044 home accounts owned; only 1 split-ownership domain in the entire base — decision 15 is cheap to settle."
  - 'Session closed out: (1) crontab incident RESOLVED — recovered byte-exact from the May-5 AMI via a throwaway instance, restored on master, and version-controlled as docs/ops/crontab-master.txt (commit 4c838cf9a); confirmed NO sandbox-refresh job exists on the master under any user — the refresh has been manual all along, and SandboxController now preserves customer_sales_scores (01cc78c4f). (2) Threshold & Ownership Workshop BOOKED: M-009, Mon 6 Jul 14:00-16:00, Ben/Austin/Nathan/Sam, agenda = the full decision docket from P-0018-workshop-pack.md (5cda65112) with the What-if panel as instrument and a record-everything close-out. (3) "How we slice it today" comparison shipped (current accountType/tier vs engine levels, per-row chips + cross-tab) and engine-version stamping on saved sets (replay-safe parameter evolution). Project state corrected planning → active.'
actions:
  ...
  - text: "Before Monday 14:00: Ben reads the workshop pack; Austin closes what he can of the review queue (T-0482, T-0480, T-0484, T-0479, T-0497 — checklists on each run) and has the sandbox What-if panel ready to project."
side_quests:
  ...
  - "salesperson table hygiene — PARKED by Austin (no data changes now, \"we don't test on prod\"): three legacy rows carry userID=0 (Taran O'Doherty s_ID 0 — has a CURRENT row s_ID 96/userID 122; Terry Badger s_ID 11; Becky s_ID 30; all active=0). They tripled the account-levels worklist via the owner join (fixed join-side in 8bbe1c2ec — deduped subquery, userID<>0). IF ever cleaned: set userID to NULL (needs the NOT NULL column relaxed first), do NOT delete — s_ID keys may anchor historical sales/management reports. Any such change is Austin's to run, sandbox first, live deliberately."
