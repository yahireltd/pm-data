...
assignee:
  ...
  name: claude-code
acceptance_criteria:
  - "[x] The right-hand duration is labelled (took Xm / Xm so far) so it can't be misread as a timestamp"
  - "[x] Sort pills on the Progress page: Latest update (default), Recently started, Most effort — selectable via links, active one highlighted"
  - "[x] Sorting by Most effort ranks longest runs first (open runs count elapsed-so-far)"
  - "[x] Default behaviour unchanged: newest update first"
out_of_scope: []
code_anchors:
