...
assignee:
  ...
  name: claude-code
acceptance_criteria:
  - pm_update_outcome edits an outcome's owner/description/due by index without touching the others; clear_owner removes the owner
  - pm_attach_meeting_transcript with replace:true swaps an existing transcript (attachment + body section) for a new one; without replace, duplicate still rejected
  - Replace excises only the matching transcript's body section, not a second audio's transcript on the same meeting
  - Used live to re-attribute M-003's outcomes and put the speaker-labelled transcript on the meeting
out_of_scope: []
code_anchors:
